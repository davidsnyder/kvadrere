...
2. @test/test_tile_geometry.sh@ runs the @test/tile_geometries.pig@ runner script and combines the output @part-*@ files into a single GeoJSON feature collection for display in a program like QGIS.

The input is @test/data/admin0_MX.geojson@, which contains a single GeoJSON Feature MultiPolygon for Mexico.
